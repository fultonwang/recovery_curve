...
#from python_utils.python_utils.stan import *
...
#        K = len(iter(recovery_X_train.x_ns).next())
        K = len(iter(recovery_X_train[1]).next())
...
#    l_c = everything_dist.B_phis_dist.phi_c_dist.horse.keywords['l']
    l_a = everything_dist.B_phis_dist.phi_a_dist.horse.args[0]
    l_b = everything_dist.B_phis_dist.phi_b_dist.horse.args[0]
    l_c = everything_dist.B_phis_dist.phi_c_dist.horse.args[0]
...
def fit_single_average_shape(s, ts, ys):
    
    def obj_f((a,b,c)):
        pred = np.array([recovery_utils.f(s_i,a,b,c,t) for t in ts])
        return (pred - d).dot(pred - d).sum()
...
class mean_unscaled_predictor(object):
    ...
    def predict(self, recovery_X_test):
        s_ns_test, x_ns_test, ts_ns_test = recovery_X_test
        return np.array([\
                [self.t_to_scaled_val_d[t] for t in ts_i]\
                for (ts_i, s_i) in itertools.izip(ts_ns_test, s_ns_test)\
                ])

        
class mean_unscaled_fitter(object):

    def fit(self, recovery_X_train, ys_ns_train):
        s_ns_train, x_ns_train, ts_ns_train = recovery_X_train
        return mean_unscaled_predictor(pd.DataFrame([pd.Series(ys_i, index=ts_i) for (s_i,ys_i,ts_i) in itertools.izip(s_ns_train, ys_ns_train, ts_ns_train)]).mean())
...
class unscaled_logreg_predictor(object):
    ...
    def predict(self, recovery_X_test):
        s_ns_test, x_ns_test, ts_ns_test = recovery_X_test
        return np.array([\
                         1. * np.array([self.Bs[t].dot(x_i) for t in ts_i])\
                         for (s_i,x_i,ts_i) in itertools.izip(s_ns_test,x_ns_test,ts_ns_test)\
                         ])


class unscaled_logreg_fitter(object):
...
#            pdb.set_trace()
            ...
            def obj_f(B):
#                print logistic(x_ns_train.dot(B)) * s_ns_train
                return (pd.Series(x_ns_train.dot(B) * 1, index=all_idxs) - vals).apply(lambda x:x**2).sum()
...
class sk_unscaled_logreg_predictor(object):
    ...
    def predict(self, recovery_X_test):
        s_ns_test, x_ns_test, ts_ns_test = recovery_X_test
        ans = np.array([\
                         1. * np.array([self.Bs[t].predict([x_i])[0] for t in ts_i])\
                         for (s_i,x_i,ts_i) in itertools.izip(s_ns_test,x_ns_test,ts_ns_test)\
                         ])
...
class sk_unscaled_logreg_fitter(object):
    ...
    def fit(self, recovery_X_train, ys_ns_train):
        ...
        for (t,vals) in d.T.iterrows():
            import pdb
            actual_vals = vals[vals.isnull()==False]
            f = linear_model.LinearRegression()
            f.fit(x_ns_train[actual_vals.index,:], actual_vals.values)

            Bs[t] = f


        return sk_unscaled_logreg_predictor(Bs)
...
class in_sample_recovery_data_cv(object):

    def __init__(self):
        pass

    def __call__(self, recovery_X, ys_ns):
        return (((recovery_X, ys_ns),(recovery_X, ys_ns)),)
